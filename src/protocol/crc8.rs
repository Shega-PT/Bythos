//! # CRC-8/SMBUS — Implementação Puro Rust
//!
//! Implementação do algoritmo CRC-8 utilizando o polinómio SMBUS (0x07).
//! Esta implementação é totalmente portátil, sem dependências externas,
//! e utiliza uma tabela lookup de 256 entradas para cálculo O(1) por byte.
//!
//! ## Especificação
//!
//! - **Polinómio:** 0x07 (x^8 + x^2 + x + 1)
//! - **Valor inicial:** 0x00
//! - **Reflexão de entrada:** Não
//! - **Reflexão de saída:** Não
//! - **XOR final:** 0x00
//!
//! ## Referência
//!
//! Este CRC é utilizado em todas as mensagens do protocolo TLV para
//! garantir a integridade dos dados em transmissão.

/// Tabela lookup de 256 entradas para CRC-8/SMBUS (polinómio 0x07).
///
/// Pré-calculada para eficiência máxima. Cada entrada[i] representa
/// o CRC-8 do byte `i` considering o polinómio 0x07.
const CRC8_TABLE: [u8; 256] = [
    ,
];

/// Calcula o CRC-8/SMBUS de um array de bytes.
///
/// Utiliza uma tabela lookup para cálculo eficiente (O(1) por byte).
/// O valor inicial é 0x00, sem reflexão, sem XOR final.
///
/// # Arguments
///
/// * `data` - Slice de bytes sobre o qual calcular o CRC.
///
/// # Returns
///
/// O valor CRC-8 resultante (0x00-0xFF).
///
/// # Examples
///
/// ```rust
/// use bythos::protocol::crc8::calc_crc8;
///
/// let data = [0x01, 0x02, 0x03];
/// let crc = calc_crc8(&data);
/// assert!(crc <= 0xFF);
/// ```
pub fn calc_crc8(data: &[u8]) -> u8 {
    let mut crc: u8 = 0x00;
    for &byte in data {
        crc = CRC8_TABLE[(crc ^ byte) as usize];
    }
    crc
}

/// Calcula o CRC-8/SMBUS de um array de bytes com tamanho fixo.
///
/// Versão para arrays de tamanho conhecido em compile-time,
/// evitando overhead de slice.
///
/// # Arguments
///
/// * `data` - Array de bytes.
///
/// # Returns
///
/// O valor CRC-8 resultante.
#[deprecated(since = "3.0.0", note = "Utilizar calc_crc8 em vez de calc_crc8_fixed")]
pub fn calc_crc8_fixed(data: &[u8]) -> u8 {
    let mut crc: u8 = 0x00;
    for &byte in data {
        crc = CRC8_TABLE[(crc ^ byte) as usize];
    }
    crc
}

/// Verifica se um CRC-8 é válido para os dados fornecidos.
///
/// # Arguments
///
/// * `data` - Dados sobre os quais verificar o CRC.
/// * `expected_crc` - CRC esperado.
///
/// # Returns
///
/// `true` se o CRC calculado corresponde ao CRC esperado.
pub fn verify_crc8(data: &[u8], expected_crc: u8) -> bool {
    calc_crc8(data) == expected_crc
}

/// Acessa a tabela lookup de CRC-8.
///
/// Útil para debug e testes. Retorna um slice de 256 bytes.
pub fn get_crc8_table() -> &'static [u8; 256] {
    &CRC8_TABLE
}

// ============================================================================
// TESTES UNITÁRIOS
// ============================================================================

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn test_crc8_empty_data() {
        // CRC-8 de dados vazios deve ser 0x00 (valor inicial)
        assert_eq!(calc_crc8(&[]), 0x00);
    }

    #[test]
    fn test_crc8_single_byte() {
        // CRC-8 de [0x00] = CRC_TABLE[0x00 ^ 0x00] = CRC_TABLE[0x00] = 0x00
        assert_eq!(calc_crc8(&[0x00]), 0x00);
        // CRC-8 de [0x01] = CRC_TABLE[0x00 ^ 0x01] = CRC_TABLE[0x01] = 0x07
        assert_eq!(calc_crc8(&[0x01]), 0x07);
    }

    #[test]
    fn test_crc8_known_vectors() {
        // Valores conhecidos CRC-8/SMBUS (polinómio 0x07, init 0x00)
        // Standard test vector: "123456789" should produce 0xF4
        assert_eq!(calc_crc8(b"123456789"), 0xF4);
        assert_eq!(calc_crc8(&[0x00, 0x00, 0x00, 0x00]), 0x00);
    }

    #[test]
    fn test_crc8_incremental() {
        // O CRC de N bytes deve ser igual ao CRC incremental
        let data1 = [0x01, 0x02];
        let data2 = [0x03];
        let combined = [0x01, 0x02, 0x03];

        let crc1 = calc_crc8(&data1);
        // CRC incremental: aplicar CRC_TABLE sobre o CRC anterior com novos bytes
        let mut crc_inc = crc1;
        for &byte in &data2 {
            crc_inc = CRC8_TABLE[(crc_inc ^ byte) as usize];
        }

        assert_eq!(crc_inc, calc_crc8(&combined));
    }

    #[test]
    fn test_verify_crc8() {
        let data = [0x01, 0x02, 0x03];
        let crc = calc_crc8(&data);
        assert!(verify_crc8(&data, crc));
        assert!(!verify_crc8(&data, crc.wrapping_add(1)));
    }

    #[test]
    fn test_crc8_table_lookup() {
        let table = get_crc8_table();
        assert_eq!(table[0], 0x00);
        assert_eq!(table[1], 0x07);
        assert_eq!(table.len(), 256);
    }

    #[test]
    fn test_crc8_protocol_header() {
        // Simular CRC de uma mensagem: start(0xAA) + msgID(0x10) + tlvCount(0)
        let header = [0xAA, 0x10, 0x00];
        let crc = calc_crc8(&header);
        // CRC não deve ser zero para estes dados específicos
        assert_ne!(crc, 0x00);
    }
}
